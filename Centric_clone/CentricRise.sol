...
/*This contract serves as an interface for other contracts to implement specific token-related functions. 
It cannot be deployed on its own but must be inherited by another contract */
...
contract CentricRise is BEP20 {
    /* Use followig libraries */
    using RoundMath for uint256; /* Rounding-related functions */
    /*using DateLib for uint256; /* Data-related utilities for timestamps */
    using SafeMath for uint256; /* protect against overflows and underflows manipulation */
    ...
    // Which equals 100 million and is a constant and immutable
    uint256 constant PRICE_BASE = 10**8;

    // Inital price of Rise in USD: constant and immutable
    uint256 constant INITIAL_PRICE = 8302083331; // first block to mint is 450895 - $83.04040810

    // So price is derived as follows: 83.04040810 × 10**8 =  8302083331


    // Structure of a Price Block
    // Structure is a custom data type that allows you to group together variables of different types into 
    // a single, cohesive unit (e.g. a Price Block in this case)
    struct Block {
    ...
    // Mapping stores key value pairs. Here, the 
    // key = number of hours since epoch (uint256), and
    // value = Block (the struct defined above)
    // This means that each key in the mapping (representing a specific hour) maps to a Block struct containing the relevant data for that hour.
    ...
    // r: This is the growth rate over a given period, expressed as a decimal. Suppose growth rate is 28.5%, so r = 0.285.
    // t: This is the number of time units over which the growth is compounded. In the context of the previous example, 
    // t would be the number of hours in the month.
    // Suppose a month has 30 days, t = 30 * 24 = 720 hours.
    // Solving the formule then gives an hourly growth rate of approximately 0.000357 (and monthly 0.285, including compounding effect)
    // Value after 720 hours then is: (1 + 0.000357)**720 = 1.285

    bool public priceFactorsLocked = false;

    // Events in Solidity are a way for smart contracts to communicate with the outside world, 
    //specifically with off-chain applications (like dApps). 
    //When an event is emitted, it writes data to the blockchain's transaction log, 
    //which external applications can then listen to and respond accordingly.
    event DoBalance(uint256 indexed currentHour, uint256 riseAmountBurnt);

    // Log event when users swaps CNR to CNS
    event ConvertToSwap(
    ...
    // Log event when users swaps CNS to CNR
    event ConvertToRise(
        address indexed converter,
